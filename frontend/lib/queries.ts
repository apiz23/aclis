...
export function useLeaderPerformance(leaderId: string | null) {
  return useQuery<LeaderPerformanceData>({
    queryKey: [...QUERY_KEYS.leaders, "performance", leaderId],
    queryFn: () => apiGet(`/leaders/${leaderId}/performance-data`),
    enabled: !!leaderId,
    staleTime: 60_000,
  });
}
